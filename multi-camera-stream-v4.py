...
class AsyncYOLOProcessor:
    """Separate YOLO processing thread pool to prevent blocking camera streams"""
    
    def __init__(self, max_workers=2):
        """Initialize async YOLO processor
        
        Args:
            max_workers: Number of YOLO processing threads (usually 1-2 is optimal)
        """
        ...
        # Queue for pending detections
        self.detection_queue = queue.Queue(maxsize=50)  # Prevent queue overflow
        self.results_cache = {}  # Cache recent results per camera
        self.cache_timeout = 0.5  # Cache results for 500ms
    ...
    def init_yolo(self):
        ...
        try:
            ...
            print(f"✓ Async YOLO processor initialized on {self.device} with {self.max_workers} workers")
            
        except Exception as e:
            print(f"⚠ Async YOLO initialization failed: {e}")
    
    def submit_detection(self, frame, camera_idx, timestamp):
        """Submit frame for async detection (non-blocking)"""
        if self.yolo_model is None:
            return None
            
        try:
            # Don't queue if queue is full (drop frames to prevent lag)
            if self.detection_queue.full():
                return None
                
            # Submit detection task
            future = self.executor.submit(self._detect_persons, frame, camera_idx, timestamp)
            return future
            
        except Exception as e:
            print(f"Error submitting detection for camera {camera_idx}: {e}")
            return None
    
    def _detect_persons(self, frame, camera_idx, timestamp):
        """Internal detection method (runs in thread pool)"""
        try:
            # Downscale for faster processing
            original_height, original_width = frame.shape[:2]
            scale_factor = 640 / original_width if original_width > 640 else 1.0
            ...
            # Run YOLO inference
            results = self.yolo_model(rgb_frame, conf=0.5, classes=[0], verbose=False)[0]
            ...
            print(f"YOLO detection error for camera {camera_idx}: {e}")
            return []
    
    def get_cached_results(self, camera_idx, current_time):
        """Get cached detection results if available and recent"""
        if camera_idx in self.results_cache:
            cached = self.results_cache[camera_idx]
...
class MultipleCameraBoundaryAlarmSystem:
    def __init__(self, camera_sources=[]):
        """Initialize optimized multi-camera boundary alarm system with named cameras
        ...
        self.windows_to_recreate = set()
        
        # Initialize async YOLO processor
        self.async_yolo = AsyncYOLOProcessor(max_workers=2)
        
        # Detection timing control
        self.detection_intervals = {}  # Per-camera detection timing
        self.detection_futures = {}    # Track pending detections
        ...
        # Initialize detection intervals for each camera
        for camera_idx in self.cameras.keys():
            self.detection_intervals[camera_idx] = {
                'last_detection_time': 0,
                'detection_interval': 0.2,  # Detect every 200ms instead of every frame
                'last_results': []
            }
    ...
    def detect_persons_async(self, frame, camera_idx):
        """Non-blocking person detection using async processor"""
        current_time = time.time()
        detection_info = self.detection_intervals.get(camera_idx, {})
        
        # Check if we need to run detection
        time_since_last = current_time - detection_info.get('last_detection_time', 0)
        detection_interval = detection_info.get('detection_interval', 0.2)
        
        if time_since_last >= detection_interval:
            # Submit new detection (non-blocking)
            future = self.async_yolo.submit_detection(frame, camera_idx, current_time)
            if future:
                self.detection_futures[camera_idx] = future
                detection_info['last_detection_time'] = current_time
        
        # Try to get results from completed detection
        if camera_idx in self.detection_futures:
            future = self.detection_futures[camera_idx]
            if future.done():
                try:
                    results = future.result(timeout=0.001)  # Non-blocking check
                    if results is not None:
                        detection_info['last_results'] = results
                        self.detection_intervals[camera_idx] = detection_info
                    del self.detection_futures[camera_idx]
                except (concurrent.futures.TimeoutError, Exception):
                    pass  # Detection still running, use cached results
        
        # Return cached results or empty list
        return detection_info.get('last_results', [])
    ...
    def check_boundary_crossing(self, persons, camera_idx):
        ...
        if state['person_detection_enabled']:
    ...
    def process_camera(self, camera_idx):
        """Optimized camera processing with async detection"""
        cap = self.cameras[camera_idx]
        state = self.camera_states[camera_idx]
        
        print(f"Camera {camera_idx} optimized processing started")
        ...
        while True:
            try:
                ...
                # Read frame (this should be fast)
                ret, frame = cap.read()
                ...
                # Get persons using async detection (non-blocking)
                persons = []
                if state['person_detection_enabled']:
                    persons = self.detect_persons_async(frame, camera_idx)
                ...
                # Update frame (this should be fast)
                with self.frame_lock:
                    self.latest_frames[camera_idx] = frame
                
                if camera_idx in self.active_alert_windows:
                    with self.alert_lock:
                        self.alert_frames[camera_idx] = frame
                
                # Minimal delay for consistent frame rate
...
def main():
    try:
        ...
        print("\n" + "="*80)
        print("OPTIMIZED MULTI-CAMERA SYSTEM WITH ASYNC YOLO")
        print("="*80)
        
        system.run()
        
    except Exception as e:
        print(f"Error starting optimized system: {e}")
